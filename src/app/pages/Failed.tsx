...
export default function Failed() {
  ...
  const { amount, tableId, tableNo, billNo, terminal, errorCode, errorMessage } = location.state || {};
  const fallbackTableId = tableId || String(tableNo || '').replace(/[^\d]/g, '');
          ...
          <div className="max-w-md w-full">
            <div className="bg-white rounded-xl border border-gray-200 shadow-lg p-8">
              ...
              <div className="space-y-3">
                <Button
                  onClick={() => navigate(fallbackTableId ? `/payment/${fallbackTableId}` : '/dashboard')}
                ...
                <Button
                  onClick={() => navigate(fallbackTableId ? `/bill/${fallbackTableId}` : '/dashboard')}
                  variant="ghost"
