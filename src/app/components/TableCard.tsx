...
export function TableCard({ id, number, status, guests, total, time, reservationLabel, isMerged = false, onOpenTable }: TableCardProps) {
  const config = statusConfig[status];
  const isActive = status !== 'Boş' && status !== 'Ödendi';
  const isEmpty = status === 'Boş';
  const showReservation = isEmpty && !!reservationLabel;
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          {isMerged && (
            <div className="rounded-full bg-purple-100 px-2 py-1 text-[11px] font-semibold text-purple-700">
              Birleşik
            </div>
          )}
      ...
      {isEmpty && (
        <div className="text-center py-4">
          <div className={`mb-2 text-sm ${showReservation ? 'font-medium text-amber-700' : 'text-gray-400'}`}>
            {reservationLabel || 'Müsait'}
          </div>
